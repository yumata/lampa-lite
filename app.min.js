(function () {
  ...
  function removeNoIncludes(where, items) {
    for (var i = where.length - 1; i >= 0; i--) {
      if (items.indexOf(where[i]) === -1) remove$1(where, where[i]);
    }
  ...
  var templates = {
    ...
    activity: html$P,
    settings: html$N,
    settings_main: html$M,
    settings_interface: html$L,
    settings_player: html$K,
    settings_more: html$J,
    settings_plugins: html$I,
    settings_cloud: html$H,
    settings_account: html$G,
    scroll: html$O,
    items_line: html$F,
    card: html$E,
    player: html$D,
  ...
  var scroll$4 = new create$k({
    mask: true,
    over: true
  });
  var active$4;
  html$b.find('.selectbox__body').append(scroll$4.render());
  ...
  function bind$3() {
    ...
    active$4.items.forEach(function (element) {
      ...
      scroll$4.append(item);
    });
  }

  function show$2(object) {
    active$4 = object;
    bind$3();
  ...
  function toggle$8() {
    Controller.add('select', {
      toggle: function toggle() {
        var selected = scroll$4.render().find('.selected');
  ...
  function save$3(method, type, card) {
    ...
    if (account.token && Storage.field('account_use')) {
      ...
      network$5.clear();
      network$5.silent(api + 'bookmarks/' + method, update$6, false, {
        type: type,
  ...
  function toggle$7(where, card) {
    read$1();
    var find = cloud(card);
    if (find[where]) remove(where, card);else add$6(where, card);
  ...
  var baseurl$1 = Utils.protocol() + 'api.themoviedb.org/3/';
  var baseimg = Utils.protocol() + 'image.tmdb.org/t/p/w200/';
  var network$4 = new create$l();
  ...
  function relise() {
    ...
    var oncomplite = arguments.length > 1 ? arguments[1] : undefined;
    var onerror = arguments.length > 2 ? arguments[2] : undefined;
    network$2.silent(Utils.protocol() + 'tmdb.cub.watch?sort=releases&results=40&page=' + params.page, function (json) {
  ...
  function infinite(object, params) {
    ...
    function append(element) {
      ...
      item.create();
      item.render().on('hover:enter', function (e) {//scroll.update($(e.target),true)
      });
    ...
    function display() {
      var position = 5 * row;
      render.slice(position, position + 10).forEach(function (item) {
        body.append(item);
      });
      items.slice(position, position + 10).forEach(function (card) {
        card.visible();
      });
      ...
      var maxrow = Math.ceil(items.length / 5) - 2;
      if (position / 5 >= maxrow) params.onNext(function (data) {
  ...
  function component$6(object) {
    ...
    this.build = function (data) {
      info = new create$h();
      info.create();
      scroll = new infinite(object, {
        data: data.results,
        info: info.render(),
        onNext: function onNext(back) {
          if (waitload) return;
        ...
        onItem: function onItem(card, element) {
          card.onFocus = function (target, card_data) {
            last = target;
            info.update(card_data);
          };
    ...
    this.start = function () {
      Controller.add('content', {
        toggle: function toggle() {
          Controller.collectionSet(html);
          Controller.collectionFocus(last || false, html);
        },
        left: function left() {
          if (Navigator.canmove('left')) Navigator.move('left');else Controller.toggle('menu');
        },
  ...
  function component$4(object) {
    ...
    this.more = function () {
      var more = $('<div class="category-full__more selector"><span>Еще</span></div>');
  ...
  var subtitles = html$9.find('.player-video__subtitles');
  var timer$3 = {};
  ...
  function create$b() {
    ...
    if (Platform.is('tizen') && Storage.field('player') == 'tizen') {
      videobox = create$e(function (object) {
        _video = object;
      });
    } else {
  ...
  var listener$7 = start$2();
  var network$1 = new create$l();
  var elems = {
  ...
  function stat$1(url) {
    ...
    var update = function update() {
      ...
      network$1.silent(url.replace('preload', 'stat').replace('play', 'stat'), function (data) {
      ...
      network.silent(link, function (result) {
        ...
        if (result.trim()) {
          ...
          body.find('.selector').on('hover:focus', function () {
            ...
            var json = Arrays.decodeJson($(e.target).attr('data-json'), {});
              ...
              Select.show({
                ...
                onSelect: function onSelect(a) {
                  if (a.method == 'play') {
                    Player.play(a);
                    Player.playlist([json.url]);
                  } else if (a.method == 'link' && a.url) _this3.get(a.url, true);
                },
                onBack: function onBack() {
        ...
        } else _this3.empty('Сервер вернул пустой результат');
      }, function () {
        _this3.empty();
      }, false, {
        dataType: 'html'
    ...
    this.toggle = function () {
      ...
      Controller.add('view_videos', {
        toggle: function toggle() {
          ...
          Controller.collectionFocus(last || body.find('.focused')[0], _this5.render());
        },
        right: function right() {
          Navigator.move('right');
        },
        ...
        up: function up() {
          if (Navigator.canmove('up')) Navigator.move('up');else _this5.onUp();
        },
        gone: function gone() {},
        back: function back() {
          if (history.length > 1) {
            ...
            _this5.get(history[history.length - 1], false);
          } else _this5.onBack();
        }
      });
      Controller.toggle('view_videos');
  ...
  function edit(params, call) {
    ...
    if (params.free) html$5.addClass('settings-input--free');
    $('body').append(html$5);
    keyboard$1 = new create$7();
  ...
  var html$4, active, scroll$3, last$1;

  function open(params) {
    active = params;
    html$4 = Template.get('modal', {
      title: params.title
    });
    html$4.on('click', function (e) {
      if (!$(e.target).closest($('.modal__content', html$4)).length) window.history.back();
    });
    title(params.title);
    html$4.toggleClass('modal--medium', params.size == 'medium' ? true : false);
    html$4.toggleClass('modal--large', params.size == 'large' ? true : false);
    scroll$3 = new create$k({
      over: true,
      mask: params.mask
    });
    html$4.find('.modal__body').append(scroll$3.render());
    bind$1(params.html);
    scroll$3.append(params.html);
    $('body').append(html$4);
    toggle();
  }
  ...
  function toggle() {
    Controller.add('modal', {
      invisible: true,
      toggle: function toggle() {
        Controller.collectionSet(scroll$3.render());
        Controller.collectionFocus(last$1, scroll$3.render());
      },
      up: function up() {
        Navigator.move('up');
      },
  ...
  function update$3(new_html) {
    last$1 = false;
    scroll$3.clear();
    scroll$3.append(new_html);
    bind$1(new_html);
    toggle();
  }

  function title(tit) {
    html$4.find('.modal__title').text(tit);
    html$4.toggleClass('modal--empty-title', tit ? false : true);
  }
  ...
  function create$5(data) {
    ...
    this.append = function (element) {
      ...
      element.ready = true;
      var card = new create$i(element, params);
      card.create();
  ...
  function create$4() {
    ...
    this.build = function (data, type) {
      ...
      var item = new create$5(data, params);
      item.onDown = this.down;
      item.onUp = this.up;
